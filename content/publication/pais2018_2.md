...
"""
abstract_short = """
In this work, we are introducing a new research field (Algerian dialects identification), which is still in its infancy. More specifically, we briefly highlight the socio-linguistics situation in Algeria, we present an attempt to cluster Algerian dialects over Algeria's map. Finally, we evaluate and compare the state-of-the-art tools of language identification on this sub-field, where the experimental results figured out that the tackled task is more difficult and requires more investigation.
"""
